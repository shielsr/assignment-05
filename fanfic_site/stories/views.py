...
class ChapterDetailView(DetailView):
    ...
    def get_object(self):
        ...
        return get_object_or_404(Chapter, story_id=story_id, number=number)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["title"] = (
            f"{self.object.story.title} - Chapter {self.object.number}: {self.object.title}"
        )
        return context
...
class UpdateChapter(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def get_success_url(self):
        return redirect(
            "chapter-detail", story_id=self.object.story.id, number=self.object.number
        ).url
...
class StoryCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        return super().form_valid(
            form
        )  # Validate the form by running form_valid method from the parent class
...
def TogglePublish(request, pk):
    ...
    if story.status == "draft":
        ...
        if (
            not story.date_published
            or story.date_published == story._meta.get_field("date_published").default
        ):
