...
class ComposeMailView(LoginRequiredMixin, CreateView):
    ...
    def get_initial(self):
        ...
        recipient_username = self.kwargs.get(
            "recipient_username"
...
class MailboxView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return Mail.objects.filter(
            recipient=self.request.user, is_archived=False
        ).order_by("-timestamp")
...
def unarchive_mail(request, pk):
    ...
    if mail.recipient != request.user:
        return HttpResponseForbidden(
            "You don't have permission to unarchive this mail."
        )
...
class ArchivedMailView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return Mail.objects.filter(
            recipient=self.request.user, is_archived=True
        ).order_by("-timestamp")
