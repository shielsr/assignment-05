...
Root Directory: fanfic_site/  (Note: This depends on what folder you upload to Github and may not be necessary)

Build Command: uv sync && uv run manage.py collectstatic && uv run manage.py migrate && uv run manage.py ensure_adminuser

Start Command: gunicorn fanfic_site.wsgi:application
